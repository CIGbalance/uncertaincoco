 ...
 * Author: thehedgeify
 *
 * Created on 11 August 2018, 14:22
 */
...
void evaluateFunction() {
    ...
    f.setUncertaintyThreshold(0.05);
    std::cout<<"evaluating "<<f.name()<<" ";
    result = f.eval(point);
    std::cout << result << std::endl;   
    std::cout << "number of evaluations, should be 3: " << f.getEvaluations() << std::endl;
    std::cout << "number of real Evaluations, should be 101: " << f.getRealEvaluations() << std::endl;
...
void testCMA(){
    ...
    f1.init(problem, problem2);
    f1.lhs(100, 17, 5);
    Kriging_pred_s optimFunction;
    optimFunction.init(&f1, 10, 0.05, 0.05);
...
void testArchiving(){
    ...
    f.setHistoryThreshold(60);
    f.setUncertaintyThreshold(1);
    std::cout<<"evaluating "<<f.name()<<" ";
    point(0)=2;
    result = f.eval(point);
    std::cout << result << std::endl;  
    std::cout << "Archive data size should be 2: " << f.getArchive().getNoPointsCurrent() << std::endl;
